...
- agent repair loop (`packages/mcp-server/test/agent-repair-loop.test.ts`) authors a marker through the command bus, runs real Electron acceptance, observes the structured position failure, patches the transform, and passes a second acceptance run including a valid PNG. The temp project is removed when the test finishes. Linux runs only when `DISPLAY` is set. Proof: that test passed inside `pnpm check` on Linux with `DISPLAY` set (REAL ELECTRON, dev host, `isPackaged` false). It is not packaged `KinetraGame` proof. After `canRunRealElectronTests` lands, this branch should call that helper instead of repeating the display condition.
