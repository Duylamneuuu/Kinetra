...
| P5 animation | HUMANOID RETARGET BAKE & CACHE SLICES PROVEN | Real external humanoid/skinned GLB (`CesiumMan.glb`, CC-BY 4.0) ingestion, skeleton hierarchy inspection via `@gltf-transform/core`, semantic humanoid bone mapping across differing artist names, delta-quaternion transform retarget baking (`bakeRetargetedClip`) generating target-specific `THREE.AnimationClip` on `EnemyBot`, deterministic retarget cache key, engine-owned persistent bake cache (`RetargetBakeCache`, `FileRetargetCacheStorage`) with multi-process cache HIT reuse, automatic invalidation across changed inputs/signatures, safe corruption/stale schema recovery, dynamic runtime clip registration, real Electron playback and stepping advancing bone rotations, and valid PNG visual capture. In addition, rigged character combat animation is proven with 7-bone hierarchy and 6 combat clips driven by combat state machine. Animation graph runtime, blend trees, and physics-driven root motion remain unfinished. |
