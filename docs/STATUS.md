...
- full real-Electron verification suite (runtime, physics, navigation, GLB models, animation, gameplay scripts, save/load, audio, desktop save, arena, game shell, gameplay loop, combat) is gated by `canRunRealElectronTests` (Windows always; Linux when `DISPLAY` is set; other platforms skip). Verification hosts pass `realElectronLaunchArgs()` (`--no-sandbox` and software rendering) only on Linux; the default `ElectronRuntimeHost` does not. Local proof: LINUX CLOUD, `pnpm --filter @kinetra/verification test` with `DISPLAY` set, 65 passed / 0 failed / 0 skipped. Linux CI runs the same command under xvfb (`.github/workflows/linux-verification.yml`). This is dev Electron, not packaged `KinetraGame` proof. The 2026-09-22 CI run aborted with `SIGTRAP` before these switches existed.
