...
- player runtime log retention is capped at 2000 entries. Overflow drops the oldest `debug`, then `info`, then `warning`, then `error`, and sequence numbers keep increasing. `readLogs` still returns the retained array. `runtime.metrics` reports `logsDropped` (0 for a new controller, increasing only when an entry is dropped, and kept across `runtime.stop` / `runtime.start` because the buffer is process-lifetime). Proof level: UNIT (`BoundedLogBuffer`) plus REAL ELECTRON under xvfb (`real-log-metrics`, 1 passed / 0 failed): a fresh process reports `logsDropped: 0`, stop/start keeps earlier logs with the count still 0, 2100 `player.jump` injections raise `logsDropped` above 0 while `readLogs` stays an array of at most 2000 entries with no dropped field, a new process starts again at 0, and `/proc` shows no leftover process for the user-data directory;
