...
- a missing `assert.equal` / `assert.near` path names the absent key and up to 12 sibling keys instead of reporting `undefined`. Proof level: acceptance-runner unit test with `FakeProbe`. This is not Electron or packaged-executable proof;
